...
def test_jobrow_from_running_line() -> None:
    line = "   1234567   cpufast    vscode        R       00:05      04:00:00      1 n01"
...
def test_jobrow_from_pending_line_keeps_reason() -> None:
    line = "   1234568   cpufast    vscode       PD       0:00       04:00:00      1 (Resources)"
    r = JobRow.from_squeue_line(line)
    assert r is not None
    assert r.state == "PD"
...
async def test_refresh_populates_table(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda cfg: (
            ...
            "1234567 cpufast vscode R 00:05 04:00:00 1 n01\n"
            "1234568 gpufast vscode-gpu R 00:10 04:00:00 1 g05\n"
        ),
    )
    monkeypatch.setattr(
