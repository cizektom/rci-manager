...
> **Heads-up:** if you also use [zsh-setup](https://github.com/cizektom/zsh-setup),
> remove the `alias rci='ssh rci'` from `.zshrc` — it shadows this binary. The new
> equivalent is `rci ssh`.
...
| ------------------------ | ------------------------- | -------------------------------------------------- |
| `rci ssh`                | `rci` (alias `ssh rci`)   | interactive shell on the login host                |
